...
import { Users2, Plus, Trash2, ChevronDown, ChevronUp, SkipForward } from 'lucide-react';

const INLAW_RELATIONSHIPS = [
  'Father-in-Law',
  'Mother-in-Law',
  'Step-Father-in-Law',
  'Step-Mother-in-Law',
...
export interface ParentInLawEntry {
  id: string;
  firstName: string;
  lastName: string;
  surname: string;
...
interface Step6Props {
  defaultValues?: ParentInLawEntry[];
  onNext: (data: ParentInLawEntry[] | null) => void;
  onBack: () => void;
}
...
function InLawForm({
  ...
  entry: ParentInLawEntry;
  index: number;
  onUpdate: (id: string, data: Partial<ParentInLawEntry>) => void;
  ...
  const {
    ...
    getValues,
    formState: { errors },
  } = useForm<Omit<ParentInLawEntry, 'id'>>({
    defaultValues: {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                ...
                {INLAW_RELATIONSHIPS.map((r) => (
                  <option key={`ilrel-${r}`} value={r}>
                    {r}
                  </option>
                ))}
              </select>
...
export default function Step6ParentsInLaw({ defaultValues, onNext, onBack }: Step6Props) {
  ...
  const updateEntry = (id: string, data: Partial<ParentInLawEntry>) =>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-blue-100 flex items-center justify-center">
            <Users2 size={14} className="text-blue-700" />
          </div>
          <div>
            <h3 className="text-base font-semibold text-foreground">Parents-in-Law Details</h3>
            <p className="text-xs text-muted-foreground">
              Add your parents-in-law. This step is optional.
            </p>
          </div>
        </div>
      ...
      >
        <Plus size={16} /> Add Another Parent-in-Law
      </button>
